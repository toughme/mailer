...
function createCampaignsService({ db, schedulerService, sendQueueService }) {
  ...
  async create(payload) {
    ...
    await db.run(
      ...
      (name, subject, preview_text, content, subject_b, content_b, ab_enabled, split_ratio, status, segment_id, recipient_ids, use_individual_recipients, scheduled_at, reply_to, timezone, metrics, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
      [name, subject, previewText, content, subjectB, contentB, abEnabled ? 1 : 0, splitRatio, status, segmentId, JSON.stringify(recipientIds), useIndividualRecipients ? 1 : 0, scheduledAt, replyTo, timezone, metrics]
    );
  ...
  async update(payload) {
    ...
    await db.run(
      `UPDATE campaigns
      SET name = ?, subject = ?, preview_text = ?, content = ?, subject_b = ?, content_b = ?, ab_enabled = ?, split_ratio = ?, segment_id = ?, recipient_ids = ?, use_individual_recipients = ?, scheduled_at = ?, reply_to = ?, timezone = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?`,
      [name, subject, previewText, content, subjectB, contentB, abEnabled ? 1 : 0, splitRatio, segmentId, JSON.stringify(recipientIds), useIndividualRecipients ? 1 : 0, scheduledAt, replyTo, timezone, id]
    );
